public class LinearArray{
  ...
  public static int [] shiftLeft(int [] source, int k){
    for(int i=0,p=k;p<source.length;i++,p++){
      source [i]=source[p];
    }
  ...
  public static int [] rotateLeft(int [] source, int k){
    ...
    for(int j=0 ;j<source.length-k;j++ ){
      source[j]=source[j+k];
    }
    for(int j=source.length-k,n=0;j<source.length;j++,n++){
      source[j]= c[n];
    }
    return  source; 
  }
  ...
  public static int [] rotateRight(int [] source, int k){
    int [] c=new int[k ];
    for(int i=source.length-k,j=0; j<c.length;i++,j++){
      c[j]=source[i];
    }
    for(int i=source.length-1,j=source.length-k-1; j>=0;i-- ,j--){
      source[i]=source[j];
    }
...
//    */
  public static boolean insert(int [] arr, int size, int elem, int index){
    if(size>=arr.length){
      ...
      int st=arr.length-size;
      for(int i=arr.length-st;i>index;i--){
        arr[i]=arr[i-1];
      }
      arr[index]=elem;
      return true;
